<div>
    ...
    <div class="mb-2 mt-2 flex items-center justify-between">
        <form wire:submit="searchDokumenDebitur()">
    ...
    @if ($debiturResult)
        {{-- <p>Nama Debitur : {{ $debiturResult->nama_debitur }}</p>
        <p>Nomor Debitur : {{ $debiturResult->no_debitur }}</p> --}}
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    <tr
                        class="border-b-2 bg-white odd:bg-gray-100 even:bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                        <th scope="row"
                            class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $debiturResult->nama_debitur }}
                        </th>
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $debiturResult->no_debitur }}
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <table class="w-full text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
                ...
                <tbody>
                    @foreach (['PPJB', 'AJB', 'SKMHT', 'APHT', 'PH', 'SHT', 'IMB', 'Sertipikat', 'PK', 'CN'] as $jenis)
                        @php
                            $dokumen = $dokumenResult->where('jenis', $jenis)->first();
                        @endphp
                        ...
                        @if ($dokumen)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    {{ $dokumen->tanggal_terima }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $dokumen->tanggal_terbit }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $dokumen->tanggal_jatuh_tempo }}
                                </td>
                            </tr>
                        @else
                            <tr
    ...
    {{-- <div class="relative overflow-x-auto shadow-lg sm:rounded-md">
        <table class="w-full text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
            ...
            <tbody>
                @forelse ($notaris as $db)
                    <tr
                        class="border-b-2 bg-white odd:bg-gray-100 even:bg-gray-50 hover:bg-slate-100 dark:border-gray-700 dark:bg-gray-800 odd:dark:bg-gray-900 even:dark:bg-gray-800 dark:hover:bg-gray-600">
                        <th class="whitespace-nowrap px-6 py-4 font-medium text-gray-900 dark:text-white"
                            scope="row">
                            {{ $loop->index + $notaris->firstItem() }}
                        </th>
                        <td class="px-6 py-4">
                            {{ $db->kode_notaris }}
                        </td>
